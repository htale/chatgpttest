...
# 新代码
#!/bin/bash
# 增强版 BBR + TCP/UDP 优化脚本，适配 Reality / Hysteria2 / 高并发服务场景
...
echo "[1/7] 清理旧的 sysctl 配置项..."
for file in /etc/sysctl.conf /etc/sysctl.d/*.conf; do
    sed -i '/tcp_rmem/d;/tcp_wmem/d;/tcp_mem/d;/net.core.rmem_max/d;/net.core.wmem_max/d;/udp_rmem_min/d;/udp_wmem_min/d;/tcp_congestion_control/d;/default_qdisc/d;/tcp_keepalive/d;/tcp_no_metrics_save/d' "$file"
done
...
echo "[5/7] 写入 TCP+UDP+BBR 优化配置..."
cat >/etc/sysctl.d/99-bbr-enhanced.conf <<EOF
fs.file-max = 2097152
