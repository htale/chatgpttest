...
check_port_duplicate() {
    local port=$1
    if [[ -n "$port" && -n "${USED_PORTS[$port]}" ]]; then
        echo "❌ 端口 $port 已被其他转发占用，请重新输入。"
        return 1
    fi
    return 0
}
...
add_server_config() {
    while true; do
        echo -e "\n请输入目标服务器 IP（例如 12.34.56.78）："
        read -rp "目标 IP 地址：" server_ip
        [[ -z "$server_ip" ]] && break

        # TCP 端口输入
        echo -e "\n--- TCP 端口配置（可留空跳过） ---"
        read -rp "源端口（例如 9090）: " tcp_src
        if [[ -n "$tcp_src" ]]; then
            check_port_duplicate "$tcp_src" || continue
            read -rp "目标端口（例如 8080）: " tcp_dst
        ...
        # UDP 端口输入
        echo -e "\n--- UDP 端口配置（可留空跳过） ---"
        read -rp "源端口（例如 9091）: " udp_src
        if [[ -n "$udp_src" ]]; then
            check_port_duplicate "$udp_src" || continue
            read -rp "目标端口（例如 8081）: " udp_dst
        ...
        echo -e "\n是否继续添加其他服务器？输入 [yes] 继续，输入 [no] 结束："
        read -rp "你的选择：" choice
        [[ "$choice" =~ ^[Nn][Oo]$ ]] && break
    done
}
...
for target_ip in "${!SERVER_CONFIGS[@]}"; do
    IFS=":" read -r tcp_src tcp_dst udp_src udp_dst <<< "${SERVER_CONFIGS[$target_ip]}"

    [[ -n "$tcp_src" && -n "$tcp_dst" ]] && setup_port_forward "$target_ip" "tcp" "$tcp_src" "$tcp_dst"
    [[ -n "$udp_src" && -n "$udp_dst" ]] && setup_port_forward "$target_ip" "udp" "$udp_src" "$udp_dst"
done
...
for target_ip in "${!SERVER_CONFIGS[@]}"; do
    IFS=":" read -r tcp_src tcp_dst udp_src udp_dst <<< "${SERVER_CONFIGS[$target_ip]}"
    echo -e "\n目标服务器: ${target_ip}"
    [[ -n "$tcp_src" && -n "$tcp_dst" ]] && echo "TCP转发: ${SOURCE_IP}:${tcp_src} -> ${target_ip}:${tcp_dst}"
    [[ -n "$udp_src" && -n "$udp_dst" ]] && echo "UDP转发: ${SOURCE_IP}:${udp_src} -> ${target_ip}:${udp_dst}"
done
echo "----------------------------------------"
...
echo "6.iptables -t nat -D POSTROUTING -d 8.8.8.8 -p tcp --dport 18888 -j MASQUERADE"
echo "7.iptables -t nat -D POSTROUTING -d 8.8.8.8 -p udp --dport 28888 -j MASQUERADE"
echo "8.iptables -t nat -D PREROUTING 1"
